...
class UTFEED_Twitter
{
	...
	private function getTwitterHtmlFromApi() {
		$cache_key = $this->getApiHtmlCacheKey();
		$cached = get_transient( $cache_key );
		if ( false !== $cached ) {
			return $cached;
		}
		$stale = get_transient( $cache_key . '_stale' );
		$cache_minutes = $this->getApiCacheMinutes();
		$stale_hours = $this->getApiStaleHours();
		...
		switch ( $this->feed_type ) {
			case 'profile':
				$html = $this->renderProfileFromApi( $token );
				break;
			case 'single_tweet':
				$html = $this->renderSingleTweetFromApi( $token );
				break;
			default:
				$html = '';
				break;
		...
		if ( ! empty( $html ) ) {
			// Short cache + longer stale cache for rate limit fallback.
			set_transient( $cache_key, $html, max( 1, $cache_minutes ) * MINUTE_IN_SECONDS );
			set_transient( $cache_key . '_stale', $html, max( 1, $stale_hours ) * HOUR_IN_SECONDS );
			return $html;
		}
		return is_string( $stale ) ? $stale : '';
	}

	private function getApiHtmlCacheKey() {
		$parts = array(
			'v1',
			(string) $this->feed_type,
			(string) $this->handle,
			(string) $this->theme,
			(string) $this->feed_lang,
		);
		return 'utfeed_api_html_' . md5( implode( '|', $parts ) );
	}

	private function getApiCacheMinutes() {
		if ( function_exists( 'utfeed_get_settings' ) ) {
			$settings = utfeed_get_settings();
			if ( isset( $settings['cache_minutes'] ) ) {
				return max( 1, absint( $settings['cache_minutes'] ) );
			}
		}
		return 10;
	}

	private function getApiStaleHours() {
		if ( function_exists( 'utfeed_get_settings' ) ) {
			$settings = utfeed_get_settings();
			if ( isset( $settings['stale_hours'] ) ) {
				return max( 1, absint( $settings['stale_hours'] ) );
			}
		}
		return 6;
	}
